			 ...
			 uint64_t flags)
{
	...
	if (is_caller_non_secure(flags)) {
		...
		assert(handle == cm_get_context(NON_SECURE));
	        ...
	        //cypbest
	        pg_enter_to_os(false);
                
                //cypbest
                switch (smc_fid) {
                case TEESMC_PLAYGROUND_CREATE_SEC_OS:
                        x1 = pg_create_or_destroy_os(x1, x2, x3, true);

			ns_cpu_context = cm_get_context(NON_SECURE);
			cm_set_next_eret_context(NON_SECURE);
                        
			SMC_RET4(ns_cpu_context, x1, 0, 0, 0);
                        break;
                case TEESMC_PLAYGROUND_CHANGE_SEC_OS:
                        pg_switch_os(x1);
 
			ns_cpu_context = cm_get_context(NON_SECURE);
			cm_set_next_eret_context(NON_SECURE);
                        
			SMC_RET4(ns_cpu_context, 0, 0, 0, 0);
                        break;
                case TEESMC_PLAYGROUND_DESTROY_SEC_OS:
                        x1 = pg_create_or_destroy_os(x1, x2, x3, false);
